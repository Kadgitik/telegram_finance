...
import { ChartColumnBig, History, House, Wallet } from "lucide-react";
...
export default function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-[var(--tg-theme-section-separator-color,#333)] bg-[var(--app-secondary)] safe-pb">
      <div className="grid grid-cols-4 py-2 max-w-lg mx-auto">
        {tabs.map(({ to, label, icon: Icon }) => (
          <NavLink key={to} to={to}>
            {({ isActive }) => (
              <motion.div
                whileTap={{ scale: 0.92 }}
                className={`flex flex-col items-center justify-center py-1 text-[11px] leading-tight min-w-0 ${
